...
for(i in 1:number_of_permutations)
{
  ...
  # the below two lines effectively randomly applies the extrinsic and intrinsic labels to the observations. 
  One = scramble[1:(samplesize/2)]; # give the first 23 the extrinsic label
  Two = scramble[(samplesize/2+1):samplesize]; #give the rest the intrinsic label. 
  diff = mean(One)-mean(Two); # find the difference in sample means from current permutation. under the equality assumption, most of the time this should be close to zero
...
for(i in 1:number_of_permutations)
{
  ...
  Two = scramble[6:10]; #give the rest the intrinsic label. 
  diff = mean(One)-mean(Two); # find the difference in sample means from current permutation. under the equality assumption, most of the time this should be close to zero
  xbarholder[i] = diff; # remember the difference in sample means to compare to the one we observed in the study... 
